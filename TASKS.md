Choose one of the following tasks to work on (by random choice, use awk as a random dice rolling tool `awk -v min=1 -v max=7 'BEGIN{srand(); print int(min+rand()*(max-min+1))}'`).
