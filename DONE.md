...
## 73. Advanced Linear Algebra (Kronecker Product, Outer Product, Cross Product, and Vector/Matrix Norms)
* **What was done**:
  * **Kronecker Product (`kron`)**: Implemented high-performance Kronecker product supporting arbitrary dimensions. If inputs have different ranks, the smaller array is padded with 1s on the left (NumPy behavior). Offloaded the core computations to optimized native C heap loops (`s_kron_<type>`), supporting all 9 DTypes (float64, float32, int64, int32, uint8, int16, complex128, complex64, boolean) with zero-allocation, strided view layouts support. Upcasts input arrays to target promoted type using `_resolveDType` and supports `{NDArray? out}` recyclers.
  * **Vector Outer Product (`outer`)**: Implemented vector outer product. If inputs are not 1D, they are automatically flattened first (NumPy behavior). Computes outer products directly inside native C (`s_outer_<type>`), supporting all 9 DTypes, strided layouts, and `{NDArray? out}` recyclers.
  * **Vector Cross Product (`cross`)**: Implemented multi-dimensional cross product. Supports 2D and 3D vector calculations along specified axes (defaulting to the last axis). Offloaded core cross products along vector axes directly to native C strided cross kernels (`s_cross_3d_<type>` and `s_cross_2d_<type>`), supporting all 9 DTypes. Leverages dynamic stacking and recursive dimension sweeps in Dart for optimal performance.
  * **Vector & Matrix Norms (`norm`)**: Implemented comprehensive vector and matrix norm calculations, perfectly matching NumPy specifications.
    * For vectors (1D axis): supports L1, L2 (Euclidean magnitude), Chebyshev infinity norms (max absolute values), negative infinity norms (min absolute values), and arbitrary real $p$-norms. Offloaded core L1, L2, Lp, and infinity vector calculations directly to native C strided reductions (`r_norm_<op>_<type>`), supporting complex numbers magnitudes natively.
    * For matrices (2D axis): supports Frobenius norm (L2 of flattened elements), L1 column-sum absolute max norms, row-sum absolute max norms, infinity norms, and singular value norms (`ord = 2` and `ord = -2`) computed using LAPACK-bound SVD decomposition (`svd()`).
    * Axis-wise reductions: cleanly supports axis-wise reductions along arbitrary single axes or 2-element axes lists, with full named parameter support for `keepdims` and `{NDArray? out}` recycler buffers.
