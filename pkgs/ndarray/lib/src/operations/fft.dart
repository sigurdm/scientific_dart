...
NDArray<Complex> fft(NDArray a, {int? n, int axis = -1, NDArray<Complex>? out}) {
  if (a.isDisposed) {
  ...
  final result = out ?? NDArray.zeros(outShape, targetDType);
...
/// **Example:**
/// {@example /example/fft_example.dart lang=dart}
NDArray<Complex> ifft(NDArray a, {int? n, int axis = -1, NDArray<Complex>? out}) {
  if (a.isDisposed) {
