...
/// Finds the flat index of the first element in [a] that satisfies the comparison [op] with [target].
///
...
/// - Runs in $O(1)$ memory overhead (zero allocations in Dart space, using [ScratchArena] for FFI arguments).
///
/// **Equivalent NumPy Operations:**
/// - There is no direct equivalent in NumPy that short-circuits. Typically, NumPy users do
///   `np.argwhere(cond)[0]` or `np.argmax(cond)` which evaluates the condition on the entire array and
///   allocates memory. This function is analogous to a fast, low-overhead version of `np.argmax(op(a, target))`.
///
/// **Example:**
/// {@example /example/sorting_searching_example.dart lang=dart}
