...
void runStaticTypedAddressingExample() {
  NDArray.scope(() {
    ...
    final negativeMask = arr < 0.0; // returns an NDArray<Boolean> mask
...
void runPolymorphicNumpyParityExample() {
  NDArray.scope(() {
    ...
    // NumPy Parity C: Boolean Mask criteria filtering matches to 1D!
    final flatA = a.flatten();
    final maskCondition = flatA > 7; // boolean 1D mask
    final filteredVec = flatA[maskCondition];
    print(
      'flatA[flatA > 7] (Boolean filtering vector) -> expected [8, 9, 10, 11]: ${filteredVec.toList()}',
    );

    // NumPy Parity D: Mutations via operator []= overloads!
    final data = NDArray.fromList(
      Float64List.fromList([-2.0, 10.0, -5.0, 4.0]),
      [4],
      DType.float64,
    );
