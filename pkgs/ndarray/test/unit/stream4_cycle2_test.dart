  ...
  group('Stream 4 Cycle 2 Unit Tests', () {
    test('diff: out buffer validation when n >= a.shape[targetAxis]', () {
      NDArray.scope(() {
        ...
        final validOut = NDArray<DTypeTag>.create([0], DType.float64);
        ...
        final invalidShapeOut = NDArray<DTypeTag>.create([1], DType.float64);
        expect(() => diff(a, n: 3, out: invalidShapeOut), throwsArgumentError);

        // Mismatched dtype throws ArgumentError
        final invalidDtypeOut = NDArray<DTypeTag>.create([0], DType.int32);
        expect(
    ...
    test('exponential: non-contiguous out buffer succeeds', () {
      NDArray.scope(() {
        final base = NDArray<DTypeTag>.zeros([4, 4], DType.float64);
    ...
    test('distance: chebyshev metric & NaN propagation', () {
      NDArray.scope(() {
        ...
        final x0 = NDArray<DTypeTag>.create([2, 0], DType.float64);
    ...
    test(
      ...
      () {
        NDArray.scope(() {
          ...
          final outGeom = NDArray<DTypeTag>.create([3, 1], DType.float64);
