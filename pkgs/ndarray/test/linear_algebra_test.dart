...
void main() {
  group('NDArray Advanced LAPACK Linear Algebra Decompositions Tests', () {
    group('LAPACK Inversion (inv) tests', () {
      ...
      test('Invert Float32 matrix', () => NDArray.scope(() {
      ...
      test('Singular matrix throws ArgumentError', () => NDArray.scope(() {
      ...
      test('Singular Float32 matrix throws ArgumentError', () => NDArray.scope(() {
    ...
    group('Cholesky Decomposition tests', () {
      test('Cholesky lower triangular factor and L * L^T reconstruction', () => NDArray.scope(() {
        final a = NDArray.fromList(
      ...
      test('Cholesky Float32 matrix', () => NDArray.scope(() {
    ...
    group('QR Decomposition tests', () {
      test('QR orthogonal Q and upper triangular R verification', () => NDArray.scope(() {
        final a = NDArray.fromList(
          Float64List.fromList([
            12.0,
            -51.0,
      ...
      test('QR Float32 matrix', () => NDArray.scope(() {
        final a = NDArray.fromList(
      ...
      test('QR non-contiguous view matrix', () => NDArray.scope(() {
    ...
    group('SVD Decomposition tests', () {
      test('SVD full matrices factorizations checks', () => NDArray.scope(() {
        final a = NDArray.fromList(
      ...
      test('SVD Float32 matrix', () => NDArray.scope(() {
        final a = NDArray.fromList(
      ...
      test('SVD non-contiguous view matrix', () => NDArray.scope(() {
        final parent = NDArray.fromList(
    ...
    group('Determinant (det) tests', () {
      test('Determinant of Float64 2D square matrix', () => NDArray.scope(() {
      ...
      test('Determinant of Float32 2D square matrix', () => NDArray.scope(() {
        final a = NDArray.fromList(Float32List.fromList([1.0, 2.0, 3.0, 4.0]), [
          2,
          2,
        ], DType.float32);
      ...
      test('det() throws ArgumentError on non-square matrix', () => NDArray.scope(() {
    ...
    group('Linear Solver (solve) tests', () {
      test('Solve Float64 system of equations', () => NDArray.scope(() {
      ...
      test('Solve Float32 system of equations', () => NDArray.scope(() {
      ...
      test('Solve Complex128 system of equations', () => NDArray.scope(() {
      ...
      test('Singular matrix solve throws ArgumentError', () => NDArray.scope(() {
        final a = NDArray.fromList(Float64List.fromList([1.0, 2.0, 2.0, 4.0]), [
          2,
          2,
        ], DType.float64);
      ...
      test('Solve integer matrices system (converts to Float64)', () => NDArray.scope(() {
      ...
      test('Solve Complex64 system of equations', () => NDArray.scope(() {
    ...
    group('Eigenvalues (eig) tests', () {
      test('Eigenvalues/eigenvectors of Float64 2D square matrix', () => NDArray.scope(() {
        final a = NDArray.fromList(
      ...
      test('Eigenvalues of Float32 2D square matrix', () => NDArray.scope(() {
        final a = NDArray.fromList(
      ...
      test('Eigenvalues/eigenvectors of Complex128 matrix', () => NDArray.scope(() {
        final a = NDArray<Complex>.create([2, 2], DType.complex128);
      ...
      test('Eigenvalues/eigenvectors of Complex64 matrix', () => NDArray.scope(() {
        final a = NDArray<Complex>.create([2, 2], DType.complex64);
        a.data[0] = Complex(0.0, 0.0);
        a.data[1] = Complex(1.0, 0.0);
        a.data[2] = Complex(-2.0, 0.0);
    ...
    group('Linear Algebra Validation & Error Exceptions tests', () {
      test('cholesky() throws ArgumentError on non-square matrix', () => NDArray.scope(() {
      ...
      test('qr() throws ArgumentError on non-2D tensor', () => NDArray.scope(() {
        final a = NDArray.zeros([3], DType.float64);
        expect(() => qr(a), throwsArgumentError);
      }));

      test('svd() throws ArgumentError on non-2D tensor', () => NDArray.scope(() {
        final a = NDArray.zeros([3], DType.float64);
        expect(() => svd(a), throwsArgumentError);
      }));

      test('eig() throws ArgumentError on non-square matrix', () => NDArray.scope(() {
        final a = NDArray.zeros([2, 3], DType.float64);
        expect(() => eig(a), throwsArgumentError);
      }));

      test('eig() throws UnimplementedError on boolean matrix', () => NDArray.scope(() {
        final a = NDArray.zeros([2, 2], DType.boolean);
        expect(() => eig(a), throwsUnimplementedError);
      }));

      test('solve() throws ArgumentError on non-square matrix a', () => NDArray.scope(() {
        final a = NDArray.zeros([2, 3], DType.float64);
        final b = NDArray.zeros([2], DType.float64);
        expect(() => solve(a, b), throwsArgumentError);
      }));

      test('solve() throws ArgumentError on mismatched b first dimension', () => NDArray.scope(() {
        final a = NDArray.zeros([2, 2], DType.float64);
        final b = NDArray.zeros([3], DType.float64);
        expect(() => solve(a, b), throwsArgumentError);
      }));

      test('solve() throws ArgumentError on empty/scalar b array', () => NDArray.scope(() {
        final a = NDArray.zeros([2, 2], DType.float64);
