...
void main() {
  group('Calculus Solver: trapz() composite trapezoidal integrals', () {
    ...
    test(
      ...
      () => NDArray.scope(() {
        final yFloat = NDArray<AnyFloat>.fromList(
          [1.0, 2.0, 4.0],
          [3],
          DType.float32,
        );
    ...
    test(
      ...
      () => NDArray.scope(() {
        final y = NDArray<AnyFloat>.fromList([1.0, 2.0], [2], DType.float64);
        ...
        final intArr = NDArray<AnyInt>.fromList([1, 2], [2], DType.int64);
  ...
  group('Calculus Solver: gradient() and gradientArray() N-D gradients', () {
    test(
      ...
      () => NDArray.scope(() {
        final f = NDArray<AnyFloat>.fromList(
          [1.0, 2.0, 4.0, 7.0],
          [4],
          DType.float64,
        );
    ...
    test(
      ...
      () => NDArray.scope(() {
        final f = NDArray<AnyComplex>.fromList(
          [Complex(1.0, 2.0), Complex(2.0, 3.0), Complex(4.0, 5.0)],
          [3],
          DType.complex128,
        );
    ...
    test(
      ...
      () => NDArray.scope(() {
        final f = NDArray<AnyFloat>.fromList([1.0, 2.0], [2], DType.float64);
        expect(
        ...
        final intArr = NDArray<AnyInt>.fromList([1, 3, 6], [3], DType.int64);
  ...
  group('Integer Spacing Bug Repro', () {
    ...
    test(
      ...
      () => NDArray.scope(() {
        ...
        final recycler = NDArray<AnyFloat>.zeros([2, 2], DType.float64);
  ...
  group('Calculus Preconditions and Error Handling', () {
    ...
    test(
      ...
      () => NDArray.scope(() {
        ...
        final outTrapz = NDArray<AnyFloat>.zeros([], DType.float64);
        outTrapz.dispose();
        expect(() => trapz(a, out: outTrapz), throwsStateError);

        final outGrad = NDArray<AnyFloat>.zeros([8], DType.float64);
        outGrad.dispose();
        expect(() => gradient(a, out: outGrad), throwsStateError);
        expect(() => gradientArray(a, out: [outGrad]), throwsStateError);

        final outDiff = NDArray<AnyFloat>.zeros([7], DType.float64);
        outDiff.dispose();
    ...
    test(
      ...
      () => NDArray.scope(() {
        ...
        final outGradWrongShape = NDArray<AnyFloat>.zeros([9], DType.float64);
        expect(() => gradient(a, out: outGradWrongShape), throwsArgumentError);

        final outGradWrongDtype = NDArray<AnyFloat>.zeros([8], DType.float32);
        expect(() => gradient(a, out: outGradWrongDtype), throwsArgumentError);

        final outDiffWrongShape = NDArray<AnyFloat>.zeros([8], DType.float64);
        expect(() => diff(a, out: outDiffWrongShape), throwsArgumentError);

        final outDiffWrongDtype = NDArray<AnyFloat>.zeros([7], DType.float32);
