...
void main() {
  ...
  NDArray<Object> makeSampleArray(DType dt, List<int> shape, {int seed = 1, bool nonZero = true}) {
  ...
  group('Unary Operations Deep Coverage across all 15 DTypes', () {
    ...
    test('Unary operations error handling & invalid buffer checks', () {
      NDArray.scope(() {
        ...
        final dispArr = NDArray<double>.fromList([1.0, 2.0], [2], DType.float64);
        dispArr.dispose();
  ...
  group('Binary Operations Deep Coverage across all 15 DTypes', () {
      ...
      NDArray.scope(() {
        ...
        for (final dtA in nonComplexTypes) {
          for (final dtB in nonComplexTypes) {
            final aContig = makeSampleArray(dtA, [2, 3], seed: 4, nonZero: true);
            final bContig = makeSampleArray(dtB, [2, 3], seed: 2, nonZero: true);
      ...
      NDArray.scope(() {
        ...
        for (final dtA in implementedBitwiseIntegerDTypes) {
          for (final dtB in implementedBitwiseIntegerDTypes) {
            final aContig = makeSampleArray(dtA, [2, 3], seed: 6, nonZero: true);
            final bContig = makeSampleArray(dtB, [2, 3], seed: 4, nonZero: true);
            final aTrans = makeSampleArray(dtA, [3, 2], seed: 6, nonZero: true).transpose();
            final bTrans = makeSampleArray(dtB, [3, 2], seed: 4, nonZero: true).transpose();
    ...
    test('logaddexp, logaddexp2, copysign with Broadcasting & Strided Views', () {
      NDArray.scope(() {
        final mask = NDArray<bool>.fromList([true, false, true, false, true, false], [2, 3], DType.boolean);
    ...
    test('Binary operations error cases & unsupported operands', () {
      NDArray.scope(() {
        ...
        final cCplx = NDArray<Complex>.fromList([Complex(1.0, 2.0)], [1], DType.complex128);
  ...
  group('Logical & Comparison Operations Deep Coverage', () {
    ...
    test('logical_and, logical_or, logical_xor across all 15 DTypes with Broadcasting', () {
      NDArray.scope(() {
        ...
        for (final dtA in all15DTypes) {
          for (final dtB in [DType.boolean, DType.int32, DType.float64, DType.complex128]) {
            final aContig = makeSampleArray(dtA, [2, 3], seed: 1);
            final bContig = makeSampleArray(dtB, [2, 3], seed: 2);
            final aTrans = makeSampleArray(dtA, [3, 2], seed: 1).transpose();
            final bTrans = makeSampleArray(dtB, [3, 2], seed: 2).transpose();
    ...
    test('equal, not_equal, greater, greater_equal, less, less_equal across DTypes', () {
      NDArray.scope(() {
        ...
        for (final dtA in all15DTypes) {
          for (final dtB in [DType.int32, DType.float64, DType.uint8, DType.boolean, DType.complex128]) {
            final aContig = makeSampleArray(dtA, [2, 3], seed: 1);
            final bContig = makeSampleArray(dtB, [2, 3], seed: 2);
            final aTrans = makeSampleArray(dtA, [3, 2], seed: 1).transpose();
            final bTrans = makeSampleArray(dtB, [3, 2], seed: 2).transpose();
            ...
            if (!dtA.isComplex && !dtB.isComplex) {
              ...
              expect(() => greaterEqual(aContig, bContig), throwsUnsupportedError);
  ...
  group('Bitwise Operations on Integer Types & Strided Views', () {
    ...
    test('Bitwise operations error cases & type validation', () {
      NDArray.scope(() {
        final floatArr = NDArray<double>.fromList([1.0, 2.0], [2], DType.float64);
        final intArr = NDArray<int>.fromList([1, 2], [2], DType.int32);
        final boolArr = NDArray<bool>.fromList([true, false], [2], DType.boolean);
  ...
  group('Floating Point Classification & Tolerance Suite', () {
    test('isnan, isinf, isfinite across Float, Complex, Int, and Bool types', () {
      NDArray.scope(() {
        ...
        for (final dt in [DType.float16, DType.bfloat16, ...otherIntegerDTypes, DType.boolean]) {
    ...
    test('copysign with Subnormals, Signed Zeros, Infinities, and Out Buffers', () {
      NDArray.scope(() {
        final mask = NDArray<bool>.fromList([true, false, true, false], [2, 2], DType.boolean);

        final x1 = NDArray<double>.fromList([1.0, -2.0, 3.0, -4.0], [2, 2], DType.float64);
        final x2 = NDArray<double>.fromList([-1.0, 1.0, -0.0, 0.0], [2, 2], DType.float64);
        ...
        // Float32 copysign
        final f32_1 = NDArray<double>.fromList([5.0, -6.0], [2], DType.float32);
        final f32_2 = NDArray<double>.fromList([-1.0, 1.0], [2], DType.float32);
    ...
    test('isClose & allClose with various tolerances, equalNan, Infs, and Complex', () {
      NDArray.scope(() {
        final a = NDArray<double>.fromList([1.0, 2.0, double.nan, double.infinity], [4], DType.float64);
        final b = NDArray<double>.fromList([1.000001, 2.0, double.nan, double.infinity], [4], DType.float64);
        ...
        // Complex isClose
        final cA = NDArray<Complex>.fromList([Complex(1.0, 2.0), Complex(double.nan, 1.0)], [2], DType.complex128);
        final cB = NDArray<Complex>.fromList([Complex(1.000001, 2.0), Complex(double.nan, 1.0)], [2], DType.complex128);
        ...
        final cB2 = NDArray<Complex>.fromList([Complex(1.0, 0.0), Complex(2.0, 0.0)], [2], DType.complex128);
        expect(allClose(rA, cB2), isTrue);
        expect(allClose(cB2, rA), isTrue);

        // Strided transposed isClose with where mask and out buffer
        final a2D = NDArray<double>.fromList([1.0, 2.0, 3.0, 4.0], [2, 2], DType.float64);
        final b2D = NDArray<double>.fromList([1.0, 2.0, 3.0, 4.0], [2, 2], DType.float64);
        final mask = NDArray<bool>.fromList([true, false, true, false], [2, 2], DType.boolean);
        final outClose = NDArray<bool>.zeros([2, 2], DType.boolean);
        final resClose = isClose(a2D.transpose(), b2D.transpose(), where: mask, out: outClose);
