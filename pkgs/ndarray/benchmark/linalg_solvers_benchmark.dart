...
void main() async {
  ...
  await criterion(
    ...
    (c) {
      ...
      NDArray<AnyFloat> makeInvertible(int n) {
        final rand = math.Random(42);
        final a = NDArray<AnyFloat>.zeros([n, n], DType.float64);
        for (var i = 0; i < n; i++) {
          for (var j = 0; j < n; j++) {
      ...
      c.group('1. Linear System Solvers (LAPACK dgesv & dgels)', () {
        for (final n in [50, 100, 200]) {
          ...
          final b = NDArray<AnyFloat>.ones([n, 1], DType.float64);
        ...
        const k = 50;
        final rand = math.Random(42);
        final aRect = NDArray<AnyFloat>.fromList(
          List.generate(m * k, (_) => rand.nextDouble()),
          [m, k],
          DType.float64,
        );
        final bRect = NDArray<AnyFloat>.ones([m, 1], DType.float64);
