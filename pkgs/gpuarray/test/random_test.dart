...
void main() {
  group('GpuArray Pseudo-Random Number Generation (gpuarray.random)', () {
    test('Philox4x32 uniform random sampling and 16-bit split arithmetic (rand, uniform)', () {
      ResourceScope.scope(() {
    ...
    test('Gaussian normal sampling backed by Philox Box-Muller (randn, normal)', () {
      ResourceScope.scope(() {
        rng.seed(123);
        ...
        expect(variance, closeTo(1.0, 0.15)); // Sample variance of N(0, 1) near 1
    ...
    test('Random integers and exponential backed by Philox (randint, exponential)', () {
      ResourceScope.scope(() {
        rng.seed(42);
    ...
    test('Random choice with and without replacement, and weighted p', () {
      ResourceScope.scope(() {
        ...
        expect(
          () => rng.choice(arr, size: 3, p: [-0.1, 0.5, 0.2, 0.2, 0.2]), // Negative prob
          throwsArgumentError,
        );
        expect(
