...
void main() {
  group('Terminal Cell & Matrix Tests', () {
    test('TerminalCell value equality', () {
      const c1 = TerminalCell(char: 'A', fgColor: '#FFFFFF', bgColor: '#000000');
      const c2 = TerminalCell(char: 'A', fgColor: '#FFFFFF', bgColor: '#000000');
      const c3 = TerminalCell(char: 'B', fgColor: '#FFFFFF', bgColor: '#000000');
  ...
  group('TerminalRenderer Tests', () {
    ...
    setUp(() {
      renderer = TerminalRenderer(rows: 5, cols: 20);
    });
    ...
    test('renderDeltaToString generates minimal ANSI output string', () {
      const viewport = RenderViewport(
        ...
        carets: [
          CaretPosition(x: 48, y: 0, height: 20, line: 0, column: 1),
        ],
        gutter: RenderGutter(
          width: 24,
      ...
      expect(ansiOutput, contains('\x1b[1;1H')); // Cursor position to line 1, col 1
          ...
          RenderLine(
            ...
            tokens: [
              RenderToken(
                ...
                startColumn: 0,
                endColumn: 4,
                style: RenderTokenStyle(bold: true, italic: true, underline: true),
              ),
              RenderToken(
                text: 'Plain',
                startColumn: 4,
                endColumn: 9,
                style: RenderTokenStyle(bold: false, italic: false, underline: false),
              ),
            ],
            top: 0,
            height: 20,
  ...
  group('TerminalInputParser Tests', () {
    ...
    test('parseInput decodes Delete, Alt+Key, and Ctrl+Arrow sequences without phantom Escape', () {
      ...
      expect(capturedCombos, equals([const KeyCombination('ArrowRight', ctrl: true)]));
      capturedCombos.clear();
